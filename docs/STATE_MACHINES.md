...
Локальний ban при whitelist_add надсилає ban event, щоб не лишити сайт у syncing назавжди. Polling ProfileBanList також бачить стандартні offline-бани. Плагіни з власним сховищем банів потребують окремого адаптера або команди /wlban. Стандартний `/pardon` сервера не змінює web-доступ. Розбан із синхронізацією — `/wlunban`: плагін знімає локальний бан, API переводить banned → revoked, а з поверненням у вайтліст — ще revoked → active (через дозвіл `okrip.allow_access_restoration`) і ставить whitelist_add. Прямий перехід banned → active тригер і далі забороняє.
