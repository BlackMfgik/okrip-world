...
BEGIN
  ...
  END IF;
  IF TG_OP = 'INSERT' AND (NEW.status <> 'pending' OR (EXISTS (SELECT 1 FROM player_access WHERE user_id = NEW.user_id) AND current_setting('okrip.allow_repeat_applications', true) IS DISTINCT FROM 'on')) THEN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    END IF;
    IF NEW.status = 'approved' AND OLD.status = 'pending' AND EXISTS (SELECT 1 FROM player_access WHERE user_id = NEW.user_id) AND current_setting('okrip.allow_repeat_applications', true) IS DISTINCT FROM 'on' THEN
      RAISE EXCEPTION 'Access already decided' USING ERRCODE = '23514';
    END IF;
  END IF;
  RETURN NEW;
